...
def export_pdf():
    ...
    pdf.cell(0, 10, f"Registre de la semaine du {start.date()} au {end.date()}", ln=1, align='C')
    ...
    for e in entries:
        exit_time = e['exit_timestamp'].strftime('%H:%M') if e.get('exit_timestamp') else ''
        values = [
        ...
        if e.get('signature'):
            try:
                ...
                pdf.image(io.BytesIO(img_data), x=sig_x+2, y=sig_y+2, w=widths[-1]-4, h=row_height-4, type='PNG')
            except Exception:
                pass
        pdf.ln()
